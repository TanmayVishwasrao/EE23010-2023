...
\solution
As the coin is tossed 3 times we will get 8 different outcomes.The list of possble outcomes is {HHH,HHT,HTH,THH,HTT,THT,TTH,TTT}.\\
Let us define a random variable $X$, where getting heads is success(1).
\begin{align}
X \sim \cbrak{0,1}\\
\text{where} \pr{X=1}=p
\end{align}
Here p is the probability of getting head i.e. $\frac{1}{2}$. Suppose $X_i (1 \leq i \leq n)$ represent each of the $n$ tosses. Then we can define $Z$ as:
\begin{align}
Z = \sum_{i=1}^nX_i
\end{align}
The CDF of $Z$ is
...
\end{cases}
\end{align}
For our question the value of $n$ is 3 as the coin is tossed 3 times.
\begin{enumerate}
\item all heads\\
